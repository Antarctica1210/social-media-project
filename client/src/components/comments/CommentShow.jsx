...
export default function CommentShow({comment}) {
    ...
    const delComment = async (e)=>{
        ...
        confirm({
            ...
            content:"You are going to delete comment of " + comment.userName + "'s post.",
            async onOk(){
                try {
                    const res = await axios.delete("/posts/delcomment/"+ comment.postId + "$" + comment.userId);
                    console.log(res);
                    message.success(res);
    ...
    useEffect(()=>{
        if (userImg){
            ...
            getDownloadURL(ref(storage, 'public/images/' + userImg))
                .then((url) => {
    ...
    return (
        <div style={{display: "flex", alignItems: "center", justifyContent:"space-between"}}>
            ...
            <div className='deleteComment'>
                ...
                {(currentUser._id === userId)? <DeleteOutlined onClick={delComment}/> : ""}
            </div>
        </div>
    )
}
