...
export default function EditInfo({ cover, avatar }) {
  ...
  const [relationship, setRelationship] = useState(
    currentUser.relationship ? currentUser.relationship : 3
  );
  const desc = useRef();
    ...
    if (coverImg) {
      ...
      const newPhoto = {
        ...
        coverPicture: coverImg
          ? "cover/" + new Date().getTime() + coverImg.name
          : currentUser.coverPicture,
        profilePicture: profImg
          ? "person/" + new Date().getTime() + profImg.name
        ...
        async onOk() {
          try {
            ...
            const storageRef = ref(
              storage,
              "public/images/" + newPhoto.coverPicture
            );
      ...
      confirm({
        ...
        async onOk() {
          try {
            ...
            const storageRef = ref(
              storage,
              "public/images/" + newPhoto.profilePicture
            );
      ...
      <div className="editInfoWrapper">
        <div className="pictureBlock">
          <div className="profileCoverSmall">
            ...
            {coverImg ? (
              <img
                className="profileCoverImgSmall"
              ...
              <img
                className="profileCoverImgSmall"
                src={
                  currentUser.coverPicture ? cover : PF + "person/noCover.png"
                }
                alt=""
              />
            )}
            ...
            {profImg ? (
              <Cancel
                className="shareCancelImgSmall2"
                onClick={() => setProfImg(null)}
              />
            ) : (
              ""
            )}
            {profImg ? (
              <img
                className="profileUserImgSmall"
                src={URL.createObjectURL(profImg)}
                alt=""
              />
            ) : (
              <img
          ...
          <Button
            ...
            className="submitBtn"
            onClick={changeCover}
            disabled={coverImg ? false : true}
            style={{ width: "96%", margin: "0 10px 10px 10px" }}
          >
            Change Cover Image
          </Button>
          <Button
